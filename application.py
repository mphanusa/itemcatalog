...
# User functions to facilate ownership of items
...
# Process urls based on the input
...
def showCategories():
	...
	# Get lastest category items added
	categoryItems = session.query(CategoryItem).all()

	return render_template('categories.html', categories = categories,
		categoryItems = categoryItems)

# Show all items for a category specified by catalog_id
...
# Display an item and if it's the creator, allow edit or delete buttons to show
...
# Add an item to a category, user must be logged in.
...
def addCategoryItem():
	...
	if request.method == 'POST':
		...
		if not request.form['name']:
			flash('Please add title name')
			return redirect(url_for('addCategoryItem'))

		if not request.form['artist']:
			flash('Please add a artist')
			return redirect(url_for('addCategoryItem'))

		# Add category item
		newCategoryItem = CategoryItem(name = request.form['name'], artist = request.form['artist'],
...
# Edit an item belongs to the creator
...
def editCategoryItem(catalog_id, item_id):
	...
	if creator.id != login_session['user_id']:
		flash("To edit an item, you must first log in as the item's owner.")
		return redirect('/login')
	...
	if request.method == 'POST':
		...
		if request.form['artist']:
			categoryItem.artist = request.form['artist']
		if request.form['category']:
			categoryItem.category_id = request.form['category']
		return redirect(url_for('showCategoryItem', catalog_id = categoryItem.category_id ,
...
def deleteCategoryItem(catalog_id, item_id):
	...
	# Check if logged in user is creator of category item
	if creator.id != login_session['user_id']:
		flash("To delete an iem, your must be logged in as the item's owner.")
		return redirect('/login')
...
def gconnect():
	...
	flash("You are now logged in as %s" % user_id)
